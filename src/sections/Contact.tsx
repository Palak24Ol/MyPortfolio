  ...
  return (
    <section id="contact" className="section">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 32, alignItems: 'start' }}>
        <RetroWindow title="C:\PALAK\contact.exe" delay={0.1}>
          {sent ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }} onClick={handleSubmit} style={{ background: 'var(--olive)', color: 'var(--cream)', border: 'none', padding: '12px 24px', fontFamily: 'var(--font-mono)', fontSize: 12, letterSpacing: '0.12em', cursor: 'pointer', textTransform: 'uppercase', borderRadius: 2, boxShadow: '3px 3px 0 var(--olive-dark)' }}>
                ↗ SEND MESSAGE
              </motion.button>
            </div>
          )}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} style={{ fontFamily: 'var(--font-mono)', fontSize: 13, color: 'var(--text)', lineHeight: 2 }}>
          ...
          {[
            ...
            <motion.a key={item.label} href={item.href} target="_blank" rel="noopener noreferrer" initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} whileHover={{ x: 4 }} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '12px 16px', background: 'var(--window-bg)', border: '1px solid var(--window-border)', textDecoration: 'none', boxShadow: '2px 2px 0 var(--window-border)' }}>
